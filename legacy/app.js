...
// Original bulletin renderer; history.js supplies the repaired archive.
...
function liveDelta() {
  if (!dashboard || dashboard.stale || dashboard.monitorSource === "cache" || now - dashboard.lastCheckedAt > 180000) return 0;
...
function rowRating(day) { return `<span class="${ratingClass(day.rating)}">${escapeText(day.rating || "待评价")}</span>`; }
...
async function load() {
  ...
  try {
    dashboard = normalize(await window.LiwaiHistory.load());
    try { localStorage.setItem("liwai-last-dashboard", JSON.stringify(dashboard)); } catch {}
    $("notice").hidden = !dashboard.historyError && !dashboard.historyIncomplete;
    if (!$("notice").hidden) $("noticeText").textContent = `${dashboard.historyError || '历史尚未同步完整'}。已保留 ${dashboard.history.length} 天记录，云端共 ${dashboard.historyTotal} 天；可点击重试，缺失日期不会填成零。`;
    now = Date.now(); render();
  } catch (error) {
    if (dashboard) { dashboard.stale = true; render(); }
    let cached; try { cached = localStorage.getItem("liwai-last-dashboard"); } catch {}
